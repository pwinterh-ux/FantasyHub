...
def _user_with_leagues(count=2):
    user = User(username="multi-owner", email="multi-owner@example.com")
    db.session.add(user)
    db.session.flush()
    leagues = []
    for index in range(count):
        league = League(
            user=user, mfl_id=str(5000 + index), name=f"League {index}", year=2026,
            franchise_id="0001", lineup_mode="MANUAL",
        )
        team = Team(league=league, mfl_id="0001", name="Mine")
        player_id = 1000 + index
        player = Player(
            id=player_id, mfl_id=str(player_id), name=f"Player {player_id}",
            position="WR", team="ARI",
        )
        db.session.add_all([league, team, player])
        db.session.flush()
        db.session.add(Roster(team=team, player=player, roster_status="ACTIVE", in_ir=False))
...
def test_autoset_thread_jobs_survive_commits_with_primitive_snapshots(db_app):
    with db_app.test_request_context("/lineups/auto-submit", method="POST", data={"week": "1"}):
        user, leagues = _user_with_leagues()
        user_id = user.id

        def commit_refresh(league):
            db.session.commit()  # expires ORM instances already seen while building jobs
            return True, None, "www1.myfantasyleague.com", "cookie"

        def project(_host, _mfl_id, _year, _week, player_ids, **_kwargs):
            assert not has_app_context()
            return {pid: Projection(pid, 10.0) for pid in player_ids}

        with patch.object(lineup_routes, "current_user", SimpleNamespace(id=user_id)), patch.object(
            lineup_routes, "_require_recent_sync_or_gate", return_value=None
        ), patch.object(lineup_routes, "_user_synced_leagues", return_value=leagues), patch.object(
            lineup_routes, "_refresh_lineup_roster", side_effect=commit_refresh
        ), patch.object(lineup_routes, "fetch_projected_scores", side_effect=project) as projections, patch.object(
            lineup_routes, "pick_optimal_lineup", side_effect=lambda players, *_args: [players[0][0]]
        ), patch.object(lineup_routes, "submit_lineup", return_value=(True, "OK")) as submit, patch.object(
            lineup_routes, "render_template", return_value="summary"
        ):
            assert lineup_routes.lineups_auto_submit.__wrapped__() == "summary"
    assert projections.call_count == 2
    assert submit.call_count == 2


def test_autoset_projection_exception_becomes_per_league_error(db_app):
    rendered = {}
    with db_app.test_request_context("/lineups/auto-submit", method="POST", data={"week": "1"}):
        user, leagues = _user_with_leagues()
        user_id = user.id
        failed_mfl_id = str(leagues[0].mfl_id)
        for league in leagues:
            league.roster_status_synced_at = datetime.utcnow()
        db.session.commit()

        def project(_host, mfl_id, _year, _week, player_ids, **_kwargs):
            assert not has_app_context()
            if str(mfl_id) == failed_mfl_id:
                raise RuntimeError("projection test failure")
            return {pid: Projection(pid, 10.0) for pid in player_ids}
        ...
        with patch.object(lineup_routes, "current_user", SimpleNamespace(id=user_id)), patch.object(
            ...
            lineup_routes, "fetch_projected_scores", side_effect=project
        ), patch.object(
            lineup_routes, "pick_optimal_lineup", side_effect=lambda players, *_args: [players[0][0]]
        ), patch.object(lineup_routes, "submit_lineup", return_value=(True, "OK")) as submit, patch.object(
            lineup_routes, "render_template", side_effect=capture
            ...
            assert lineup_routes.lineups_auto_submit.__wrapped__() == "summary"
    failures = [result for result in rendered["results"] if not result["ok"]]
    assert len(failures) == 1
    assert failures[0]["message"] == "Projection error: projection test failure"
    assert submit.call_count == 1


def test_batch_review_projection_worker_uses_only_snapshot_data(db_app):
    with db_app.test_request_context("/lineups/review", method="POST", data={"week": "1"}):
        ...
        with patch.object(lineup_routes, "current_user", SimpleNamespace(id=user_id)), patch.object(
            ...
            lineup_routes, "render_template", return_value="review"
        ):
            assert lineup_routes.lineups_review.__wrapped__() == "review"
    assert projections.call_count == 2


def test_batch_submission_worker_uses_only_snapshot_data(db_app):
    with db_app.app_context():
        user, leagues = _user_with_leagues()
        user_id = user.id
        data = {"week": "1"}
        for index, league in enumerate(leagues):
            data[f"include_{league.id}"] = "1"
            data[f"starters_{league.id}[]"] = str(1000 + index)
    ...
    with db_app.test_request_context("/lineups/submit", method="POST", data=data):
        with patch.object(lineup_routes, "current_user", SimpleNamespace(id=user_id)), patch.object(
            lineup_routes, "_require_recent_sync_or_gate", return_value=None
        ), patch.object(lineup_routes, "_user_synced_leagues", return_value=leagues), patch.object(
            lineup_routes, "_refresh_lineup_roster", side_effect=commit_refresh
        ), patch.object(lineup_routes, "submit_lineup", side_effect=submit) as submit_mock, patch.object(
            lineup_routes, "render_template", side_effect=capture
        ):
            assert lineup_routes.lineups_submit.__wrapped__() == "summary"
    assert submit_mock.call_count == 2
    assert all(result["ok"] for result in rendered["results"])
    assert {result["league"].id for result in rendered["results"]} == {league.id for league in leagues}
