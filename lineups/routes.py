...
def lineups_review():
    ...
    for lg in leagues:
        ...
        league_id = int(lg.id)
        league_by_id[league_id] = lg
        if _resolve_lineup_mode(lg) == LINEUP_MODE_BEST_BALL:
            jobs.append(dict(league_id=league_id, host=None, cookie=None, players=[], pid_list=[],
...
def lineups_submit():
    ...
    jobs: List[dict] = []
    league_by_id: Dict[int, League] = {int(lg.id): lg for lg in leagues}
    for lg in leagues:
        ...
        if getattr(lg, "user_id", None) != current_user.id:
            jobs.append(dict(
                league_id=int(lg.id), host=None, cookie=None, starters=[],
        ...
        if not refresh_ok:
            jobs.append(dict(league_id=int(lg.id), host=None, cookie=None, starters=[],
        ...
        jobs.append(dict(
            league_id=int(lg.id), league_mfl_id=str(lg.mfl_id), league_year=int(lg.year),
    ...
    def _submit_one(job: dict) -> Dict[str, object]:
        ...
        ok, raw = submit_lineup(
            job["host"], job["league_mfl_id"], job["league_year"], week_i,
            job["starters"], cookie=job["cookie"],
        )
    ...
    network_results = _parallel_map(_submit_one, jobs, max_workers=PARALLEL_WORKERS)
    results = [dict(result, league=league_by_id[result["league_id"]]) for result in network_results]
...
def lineups_auto_submit():
    ...
    def _fetch(job: dict) -> Tuple[int, Dict[int, Projection], Optional[str]]:
        ...
        try:
            ...
            return (league_id, proj, None)
        except Exception as exc:
            return (league_id, {}, str(exc))
    ...
    for job in jobs:
        lg = league_by_id[job["league_id"]]
        entry = proj_results.get(job["league_id"]) or {"projections": {}, "error": "Projection lookup failed"}
